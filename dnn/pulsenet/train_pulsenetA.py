...
def makearr(flist,default_pick=0):
    ...
    X=np.array(X)#.reshape(np.shape(X)[0],np.shape(X)[1],3)
    Xw=np.array(Xw)#.reshape(np.shape(Xw)[0],np.shape(Xw)[1],3)
    info=np.array(info)
#    print(np.shape(X)) # num_lc,dim,chan
#    print(np.shape(Xw)) # num_lc,dim,chan
